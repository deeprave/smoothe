# behavioral-fixture-suite Specification

## Purpose

Define black-box CLI behavioral fixtures and the dedicated runner used to
exercise `smoothe` through process-level inputs and outputs.
...
#### Scenario: Configured partial mappings are config-relative

- **WHEN** a behavioral fixture validates configured partials from a config file
  outside the template directory
- **THEN** relative configured partial paths are resolved relative to the config
